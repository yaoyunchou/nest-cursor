...
1. **问题描述**
   - 文件：`src/modules/file/file.service.ts` 第34行
   - 问题：使用 `file.mimetype.split('/')[1]` 来生成文件扩展名，导致文件 key 不正确
   - 影响：例如 `audio/mpeg` 会生成 `mpeg` 扩展名，而不是 `mp3`，导致文件 key 不准确

2. **修复内容**
   - 文件：`src/modules/file/file.service.ts`
   - 变更内容：
     - 改为从 `file.originalname` 中提取文件扩展名
     - 优先使用原始文件名中的扩展名（更准确）
     - 如果没有扩展名，则从 mimetype 中提取作为兜底方案
     - 最后兜底使用 `bin` 作为默认扩展名
...
3. **技术实现细节**
   ...
   - 提取点号后的内容作为扩展名（去掉点号）
   - 转换为小写确保一致性
   - 提供多层兜底机制，确保始终能生成有效的文件 key

4. **影响范围**
   - 修复后，七牛云文件 key 中的扩展名与原始文件扩展名一致
   - 提高文件存储的准确性和可读性
   - 与上传控制器的 mimetype 修正逻辑保持一致

## 2025-01-23（晚上 - 优化文件上传 mimetype 兼容处理）

### 以文件扩展名为准自动修正 mimetype

1. **功能说明**
   - 当上传文件的 mimetype 和文件扩展名不匹配时，自动以文件扩展名为准修正 mimetype
   - 提高文件上传的兼容性，避免因 mimetype 识别错误导致的上传失败

2. **实现内容**
   - 文件：`src/modules/reading/reading-file.controller.ts`
   - 变更内容：
     - 创建扩展名到 mimetype 的映射表：`.mp3` -> `audio/mpeg`，`.aac` -> `audio/aac`，`.wav` -> `audio/wav`
     - 优先验证文件扩展名是否在允许列表中
     - 如果扩展名在允许列表中，但 mimetype 不匹配，则根据扩展名自动修正 mimetype
     - 确保后续处理使用正确的 mimetype

3. **技术实现细节**
   - 使用 `Record<string, string>` 类型定义映射表
   - 先验证扩展名，再修正 mimetype
   - 直接修改 `file.mimetype` 属性，确保后续服务使用正确的 mimetype

4. **使用场景**
   - 某些客户端上传文件时，mimetype 可能识别错误（如 `audio/mpeg` 被识别为其他值）
   - 文件扩展名通常是准确的，以扩展名为准更可靠
   - 提高系统的容错性和兼容性

5. **影响范围**
   - 提高文件上传的成功率
   - 减少因 mimetype 不匹配导致的上传失败
   - 保持向后兼容，不影响正常上传流程
