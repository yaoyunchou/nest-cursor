/*
 * @Description: 微信公众号模板消息通知方法测试
 */
import axios from 'axios';
import { sendWechatMpNotification, WechatMpNotificationData, WechatMpNotificationResult } from './wechat-mp.notifier';

jest.mock('axios');
const mockedAxios = axios as jest.Mocked<typeof axios>;

// 暂时跳过微信公众号测试，等待appid申请
describe.skip('sendWechatMpNotification', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  describe('成功场景', () => {
    it('应该成功发送微信公众号模板消息', async () => {
      // 安排
      ...
      // 行动
      const actualResult = await sendWechatMpNotification(data);
      ...
      expect(mockedAxios.post).toHaveBeenCalledWith(
        'https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=test-access-token',
        {
          touser: 'test-openid',
          ...
          data: {
            ...
            remark: { value: '测试内容', color: '#173177' },
          },
        },
      );
    ...
    it('应该正确格式化模板数据（对象格式）', async () => {
      ...
      // 行动
      await sendWechatMpNotification(data);
  ...
  describe('错误场景', () => {
    ...
    it('应该在发送消息失败时返回错误', async () => {
      ...
      // 断言
      expect(actualResult.success).toBe(false);
      expect(actualResult.message).toContain('微信公众号模板消息发送失败');
